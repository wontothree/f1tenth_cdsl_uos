"""
Behavior Decision-Making and Lateral Motion Planner Module
"""
...
from src.motion_planner.lane_change.lane_change_trajectory_planner import LaneChangeTrajectoryPlanner
...
class BehaviorDecisionMakingAndMotionPlanner:
    ...
    def decide_behavior(self):
        ...
        if self.behavior_state == BehaviorState.LANE_KEEPING:
            if self.decide_lane_change():
                ...
                self.lane_change_motion_planner = LaneChangeTrajectoryPlanner(
                    lane_change_time=3,
                    target_y=-self.lane_width,
                    initial_state=self.ego_vehicle,
    ...
    def plan_control_command(self):
        ...
        if self.behavior_state == BehaviorState.LANE_KEEPING:
            if self.ego_vehicle[3] < self.target_speed:
                acceleration = 3
            else:
                acceleration = 0.0
            steering_angle = 0
            ...
            acceleration = 0.0
            steering_angle = self.lane_change_motion_planner.update(self.sampling_time)
