"""
Motion Planner Parking (Practice)
"""
...
class ParkingMotionPlanner:
    def __init__(self, goal_point: np.array, is_front_direction):
        ...
        self.local_coordinate_angle = np.radians(0.0)

        self.is_front_direction = is_front_direction

        # TODO (1): r_turn, r_min, r_max, x0, y0, origin_point 만들기
        self.r_turn = 0
        self.r_min = 0
        self.r_max = 0
        self.x0 = 0
    ...
    def _set_state(self, obs):
        self.heading = -obs[4]
        self.local_heading = self._change_radians_range(self.heading - self.local_coordinate_angle)
        ...
        self.rear_wheel_local_position = np.array([
            np.cos(-self.local_coordinate_angle) * position_diff[0] - np.sin(-self.local_coordinate_angle) * position_diff[1],
            np.sin(-self.local_coordinate_angle) * position_diff[0] + np.cos(-self.local_coordinate_angle) * position_diff[1]
        ])
    ...
    def _run_find_final_path(self):
        # TODO (2): run find final path 구현 하기
        steering_angle = 0
        return [0, steering_angle]
    ...
    def _run_follow_final_path(self):
        ...
        # TODO (3): steering angle 계산 로직 추가
        steering_angle = 0
