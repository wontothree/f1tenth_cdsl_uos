"""
Path Tracking Control Main (Practice)
"""
...
from src.lateral_control.linear_quadratic_gaussian_practice import LinearQuadraticGaussian
from src.lateral_control.linear_quadratic_regulator_practice import LinearQuadraticRegulator
from src.lateral_control.model_predictive_control_practice import ModelPredictiveControl
from src.lateral_control.pure_pursuit_practice import PurePursuit
from src.lateral_control.speed_controller import SpeedController
from src.lateral_control.stanley_method_practice import StanleyMethod
...
def main():
    ...
    if control_mode == "pure_pursuit":
        steering_controller = PurePursuit(env.unwrapped, gain=1, min_distance=1)
    elif control_mode == "stanley":
        steering_controller = StanleyMethod(env.unwrapped, gain=1)
    elif control_mode == "lqr":
        steering_controller = LinearQuadraticRegulator(env.unwrapped, Q=np.diag([1, 1, 1, 1]), R=np.diag([1]),
                                                       dt=sampling_time)
    elif control_mode == "lqg":
        steering_controller = LinearQuadraticGaussian(env.unwrapped, Q=np.diag([1, 1, 1, 1]), R=np.diag([1]),
                                                      W=np.diag([1, 1, 1, 1]), V=np.diag([1, 1]),
                                                      dt=sampling_time)
    elif control_mode == "mpc":
        steering_controller = ModelPredictiveControl(env.unwrapped, R=np.diag([1]), Rd=np.diag([1]),
                                                     Q=np.diag([1, 1, 1, 1, 1]), time_horizon=10,
