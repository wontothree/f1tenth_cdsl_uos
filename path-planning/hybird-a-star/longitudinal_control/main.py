...
from src.longitudinal_control.linear_quadratic_regulator import LinearQuadraticRegulator
from src.longitudinal_control.linear_quadratic_regulator_with_delay import LinearQuadraticRegulatorWithDelay
from src.longitudinal_control.model_predictive_control import ModelPredictiveControl
from src.longitudinal_control.pid_controller import PidController
...
def main():
    ...
    if control_mode == "pid":
        car_following_controller = PidController(headway_time=headway_time, distance_gains=[0.5, 0, 0],
                                                 velocity_gains=[0.1, 0, 0],
                                                 dt=sampling_time)
    elif control_mode == "lqr":
        car_following_controller = LinearQuadraticRegulator(headway_time=headway_time, Q=np.diag([2, 4]),
                                                            R=np.diag([5]),
                                                            dt=sampling_time)
    elif control_mode == "lqr_delay":
        car_following_controller = LinearQuadraticRegulatorWithDelay(headway_time=headway_time,
                                                                     Q=np.diag([2, 4, 1]),
                                                                     R=np.diag([5]), dt=sampling_time)
    elif control_mode == "mpc":
        car_following_controller = ModelPredictiveControl(headway_time=headway_time, Q=np.diag([25, 40, 1]),
                                                          R=np.diag([2]), Rd=np.diag([2]), time_horizon=10,
                                                          dt=sampling_time)
