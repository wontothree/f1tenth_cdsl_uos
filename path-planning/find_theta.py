def find_theta(current_position, track_center, trajectory_breaks, trace_width, last_closest_index):
    """
    Returns
    -------
    theta : 
    cloest_index : 현재 위치에 가장 가까운 중심선 지점의 인덱스

    Parameters
    ----------
    current_position : 현재 차량 위치 (x, y)
    track_center : 트랙 중심선 좌표 (x, y)
    trajectory_breaks : 트랙의 중단점
    trace_width : 트랙의 너비
    last_closest_index : 마지막으로 가장 가까웠던 중심선 지점의 인덱스
    """
